...
const TodoList = () => {
  const { todoList, setTodoList, setInputData, setEditedIndex, setChecked, checked } = useContext(ToDoContext);
      ...
      <div className="w-96 h-full p-4 border-amber-950 border-2 rounded-2xl">
        ...
        <div className="flex items-center justify-between gap-6">
          <div>
            ...
            <p className="font-medium text-[13px]">You Have {todoList.length} task Left</p>
          </div>
        ...
        <div className="w-full h-full mt-4 flex flex-col gap-4 overflow-hidden">
          {
            ...
            todoList.map((todo, index) => {
              return (
                <div key={index} className="w-full p-2 border-2 border-black rounded-2xl flex justify-between">
                  <div className="flex gap-4">
                    ...
                    <div>
                      <p className={`${todo.completed ? 'line-through opacity-60' : null}font-semibold text-lg`}>{todo.title}</p>
                      <p className="font-semibold text-[14px] mt-2">{todo.description}</p>
